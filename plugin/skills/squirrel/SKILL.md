...
| `customSegments` | `--custom <add\|set\|remove\|reset> <id> <field>=<value> ...` (also `--show-custom`) | fields: `text`/`command`/`when` (each takes the REST of the line), `every` (`30m`/`2h`/`90s`), `at` (`HH:MM`), `ack`/`shared` (on/off), `line` (n, default **2** — the identity/chip line; line 1 is numbers), `link` (https) | `[]` | the user's own segments — and reminders; see §4d |
